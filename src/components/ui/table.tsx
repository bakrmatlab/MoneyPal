...
function TableHead({ className, ...props }: React.ComponentProps<'th'>) {
    return (
        <th
            ...
            className={cn('text-foreground h-10 px-2 text-start align-middle font-medium whitespace-nowrap [&>[role=checkbox]]:translate-y-[2px]', className)}
            {...props}
        />
    );
}
...
function TableCell({ className, ...props }: React.ComponentProps<'td'>) {
    return <td data-slot='table-cell' className={cn('p-2 align-middle whitespace-nowrap [&>[role=checkbox]]:translate-y-[2px]', className)} {...props} />;
}
